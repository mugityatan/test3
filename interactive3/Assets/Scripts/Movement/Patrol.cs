...
public class Patrol : Physics2DObject
{
    ...
    private bool stopTrigger;   //Patrolを止めるフラグ
	...
	{
        stopTrigger = GetComponent<FollowTrigger>().isTriggered;
        //　変数stopTriggerにFollowTriggerクラスの変数isTriggeredの値を格納します 

        if (stopTrigger == false)            //もし入れられた値がfalseの場合、以下の処理（通常のパトロール）に進みます
        {
